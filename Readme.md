...
1. Copy the folder of **UTX_3117-DPS** with the generated code into the source folder **azure-iot-sdk-c-pnp** .
...
1. Open the `CMakeLists.txt` in the **azure-iot-sdk-c-pnp** folder. Include the **UTX_3117-DPS** folder so that it will be built together with the Device SDK. Add the line below to the end of the file.
    ```txt
    add_subdirectory(UTX_3117-DPS)
    ```
...
1. Once the build has succeeded, you can test it by invoking the following command.
    ```bash
    \\UTX_3117-DPS\\Release\\UTX_3117-DPS.exe
    ```
